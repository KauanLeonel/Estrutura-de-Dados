...
public class Arvore {
    ...
    public void exibirNext(NoA temp) {
      ...
      public void filhosDeUm() {
        int num = 0;
        num = filhosDeUmNext(raiz, num);
        System.out.println("Filhos De Um " + num);
    
    }

   public int filhosDeUmNext(NoA temp, int num) {
        if (temp != null) {
             num = filhosDeUmNext(temp.esq, num);
            if (countChildren(temp) == 1 ) {
                System.out.println("[" + temp.valor + "] filhos " + countChildren(temp));
       
                num++;
            }
            num = filhosDeUmNext(temp.dir, num);
        }
        return num;
    }
    ...
    public void remove(int x) {
        if (raiz.valor == x) {
            ...
            int f = countChildren(foundNoa(x));
            switch (f) {
                case -1:
...
if (temp.valor % x == 0 || temp.valor % y == 0 || temp.valor % x == z) {
